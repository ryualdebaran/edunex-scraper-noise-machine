...
const int FETCH_MS   = TEST_MODE ? 120000  : 1800000;
const int ALARM_MS   = TEST_MODE ? 60000   : 300000;
...
void connectWifi() {
  ...
  wm.setAPCallback([](WiFiManager*) {
    lcd.clear();
    lcd.setCursor(0, 0); lcd.print("1. Join WiFi:   ");
    lcd.setCursor(0, 1); lcd.print("   ESP32-Setup  ");
  });
...
void setup() {
  ...
  lastScroll = lastFetch = lastMarquee = lastBuzzer = lastFlash = millis();
}
  ...
  if (hasUnanswered && alarmActive) {
    if (now - lastBuzzer >= 2500) {
      xTaskCreate([](void*) {
        beepPanic();
        vTaskDelete(NULL);
      }, "buzz", 2048, NULL, 1, NULL);
      lastBuzzer = now;
    }
    if (now - lastFlash >= 300) {
      backlightOn ? lcd.noBacklight() : lcd.backlight();
